function WAV = doWAV(EEG,markers,baselineWindow,minimumFrequency,maximumFrequency,frequencySteps,mortletParameter)
    ...
    for conditionCounter = 1:numberOfConditions
        ...
        for epochCounter = 1:numberOfEpochs
            ...
            eventIndex = find([EEG.epoch(epochCounter).eventlatency{:}] == 0); % Epochs can contain multiple events - find the relevant one, i.e. latency 0
            thisEpochType = EEG.epoch(epochCounter).eventtype{eventIndex}; % Now get the marker at latency 0
            
            if any(strcmp(markers{conditionCounter},thisEpochType)) % CDH, In case of more than one event in epoch
